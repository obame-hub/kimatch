...
# ══ USAGE ══
#
#   VERSION PUBLIQUE, servie par Kimatch sur https://kimatch.fr/clic-pour-appeler/installer.ps1
#   Elle telecharge le script d appel au lieu de le copier depuis un depot local.
#
...
# LE SCRIPT D'APPEL SE TELECHARGE, il n'est pas a cote : cet installateur est lance depuis le
# dossier des telechargements, pas depuis le depot. Naoelle, 22/09/2026 : « tout le monde ne sait
# pas utiliser PowerShell et je suis a distance, je peux pas l'installer pour eux. »
$urlAppel = 'https://kimatch.fr/clic-pour-appeler/appeler-depuis-kimatch.ps1'
$cible = Join-Path $dossier 'appeler-depuis-kimatch.ps1'
try {
  Invoke-WebRequest -Uri $urlAppel -OutFile $cible -UseBasicParsing -ErrorAction Stop
} catch {
  Write-Output ("ECHEC: impossible de telecharger le script d'appel (" + $_.Exception.Message + ")")
  Write-Output 'Verifiez votre connexion, puis relancez Installer.bat.'
  exit 1
}
if (-not (Test-Path $cible)) {
  Write-Output 'ECHEC: le script d appel n a pas ete enregistre.'
  exit 1
}
