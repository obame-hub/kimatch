-- ════════════════════════════════════════════════════════════════════════════════════════════════
-- LE TABLEAU PRICING CONNAÎT « DEMANDE DISPONIBLE »
--
-- La vue traduit le statut de la consultation en code de colonne. Elle a été écrite avant que ce
-- statut existe : son `case` s'arrête à ENVOYEE, ACCEPTEE et REFUSEE, et son `else` renvoie
-- EN_ATTENTE. Une consultation devenue « disponible » retomberait donc dans la colonne « Demande
-- envoyée » — c'est-à-dire là où l'on attend encore une réponse du fournisseur, alors que ses offres
-- sont arrivées.
--
-- ══ LE CODE DE COLONNE PORTE LE MÊME NOM QUE LE STATUT ══
--
-- Les quatre codes de colonne (A_DEMANDER, EN_ATTENTE, RECUE, REFUSEE) ne sont pas les codes des
-- statuts : c'est une traduction héritée d'une refonte antérieure, et elle a déjà coûté cher — le
-- 28/08/2026, la colonne cherchait un code RECUE qui venait d'être supprimé, et 2 645 consultations
-- se sont retrouvées dans « En attente ».
--
-- On n'aggrave pas la traduction : le nouveau statut garde son propre code, DISPONIBLE, des deux
-- côtés. Un code de plus qui se traduit lui-même est un code de moins à retrouver le jour où ça
-- casse.
-- ════════════════════════════════════════════════════════════════════════════════════════════════
 ...
 SELECT ofr.id AS consultation_id,
    ...
    v.date_souhaitee - CURRENT_DATE AS jours_avant_cotation,
        CASE
            ...
            -- Le nouveau statut garde son code des deux côtés (Naoëlle, 01/09/2026).
