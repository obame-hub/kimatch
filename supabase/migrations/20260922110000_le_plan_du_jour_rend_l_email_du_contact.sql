-- ════════════════════════════════════════════════════════════════════════════════════════════
-- L'E-MAIL VIENT AVEC LA LIGNE, PAS PAR UNE SECONDE REQUÊTE
-- ════════════════════════════════════════════════════════════════════════════════════════════
--
-- William, 22/09/2026, sur le mode sprint : « un composant "Appeler" avec le numéro de téléphone
-- + mobile si existant et un composant "Contacter" avec le mail du contact ».
--
-- Les numéros étaient déjà rendus, pas l'adresse. On aurait pu la chercher fiche par fiche avec
-- `useFichePipe` — c'est ce que fait le volet du plan du jour — mais LE SPRINT ENCHAÎNE LES
-- FICHES : une requête par fiche ferait clignoter la carte à chaque passage, au moment précis où
-- l'on cherche le moyen de joindre quelqu'un. Une colonne de plus sur une requête déjà faite ne
-- coûte rien.
...
as $function$
  ...
  select
    ...
    -- Une piste porte son adresse en propre ; une opportunité la tient de son contact rattaché.
