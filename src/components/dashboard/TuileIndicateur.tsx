...
export function TuileIndicateur({
  ...
  return (
    // L'animation d'apparition est portée par un conteneur, PAS par la tuile. Déclarée avec un
    // fill-mode `both`, son état final (`transform: none`) persiste après la fin de l'animation, et
    // une animation CSS bat les règles normales — y compris `:hover`. Sur le même élément, elle
    // empêchait donc définitivement le `translateY(-3px)` du survol de s'appliquer.
    <div className="animate-kw-card-rise h-full" style={{ animationDelay: `${index * 0.05}s` }}>
      <button
        type="button"
        onClick={onClick}
        className="tuile-indicateur relative h-full w-full overflow-hidden rounded-2xl px-[17px] pb-4 pt-[15px] text-left"
        style={
          {
            background: `linear-gradient(145deg,${couleurHaut} 0%,${couleurBas} 100%)`,
            // L'ombre de repos passe par une variable CSS et non par `boxShadow` : en style inline
            // elle écraserait l'ombre de survol de `.tuile-indicateur:hover`, un style inline
            // primant sur une règle de classe. La transition, la courbe de William et l'ombre de
            // survol sont dans index.css — Tailwind n'exprime ni la cubic-bezier ni deux durées.
            '--ombre-repos': `0 8px 22px -12px ${couleurBas}aa`,
          } as CSSProperties
        }
      >
