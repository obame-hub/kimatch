...
export function PdlMethodSheet({
  ...
  return (
    <Sheet open={open} onClose={onClose} title="Ajouter un point de livraison" description={`Rattaché à ${compteNom}`}>
      <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-navy-400">Comment souhaites-tu procéder ?</p>
      <div className="space-y-3">
        <button
          type="button"
          onClick={() => onChoose('extraction')}
          className="group relative w-full rounded-2xl border-2 border-navy-200 bg-white p-5 text-left transition-all hover:-translate-y-0.5 hover:border-kiwi-300 hover:shadow-md"
        >
          <Badge tone="neutral" className="absolute right-3 top-3 text-[10px]">Recommandé</Badge>
          <div className="flex items-start gap-4">
            <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-kiwi-50 text-kiwi-700">
              <FileUp className="h-5 w-5" />
            </span>
            <span className="min-w-0 flex-1">
              <span className="block font-semibold text-navy-800">Extraction automatique</span>
            ...
            <ArrowRight className="mt-1 h-4 w-4 shrink-0 text-navy-300 opacity-0 transition-all group-hover:translate-x-0.5 group-hover:opacity-100" />
          </div>
        </button>
        ...
        <button
          ...
          className="group w-full rounded-2xl border-2 border-navy-200 bg-white p-5 text-left transition-all hover:-translate-y-0.5 hover:border-kiwi-300 hover:shadow-md"
        >
          <div className="flex items-start gap-4">
            ...
            <span className="min-w-0 flex-1">
              <span className="block font-semibold text-navy-800">Saisie manuelle</span>
              <span className="mt-1 block text-xs text-navy-500">
