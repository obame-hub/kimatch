...
async function fetchSlackSettings(): Promise<SlackSetting[]> {
  try {
    ...
    return replierOuRelancer(error, { ou: 'fetchSlackSettings' }, [])
  }
}
...
export function useUpdateSlackSetting() {
  ...
  return useMutation({
    ...
    /* Audit 13/09/2026, CAC-01 : le patch ci-dessus ne touche que la clé de LISTE, alors que
       les fiches lisent une clé dérivée (['parametres_slack', 'compte', id] et compagnie). Sans cette
    ...
    onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ['parametres_slack'] }) },
  })
}
