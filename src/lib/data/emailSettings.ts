...
export function useUpdateEmailSetting() {
  ...
  return useMutation({
    ...
    /* Audit 13/09/2026, CAC-01 : le patch ci-dessus ne touche que la clé de LISTE, alors que
       les fiches lisent une clé dérivée (['parametres_emails', 'compte', id] et compagnie). Sans cette
    ...
    onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ['parametres_emails'] }) },
  })
}
