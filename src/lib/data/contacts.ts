...
async function fetchContacts(compteId?: string, contactId?: string): Promise<Contact[]> {
  try {
    ...
    if (compteId) {
      ...
      /* Découpé en lots (audit 13/09/2026, ARC-02) : un syndic porte jusqu'à 1 677 sites, et
         PostgREST met chaque identifiant dans l'URL. Au-delà d'environ cent cinquante, elle devient
         trop longue et la requête échoue entièrement — les contacts rattachés par le site
...
export function useCreateContact() {
  ...
  return useMutation({
    ...
    /* Audit 13/09/2026, CAC-01 : le `setQueryData` ci-dessus ne touche que la clé de LISTE, alors
       que la fiche compte lit `['contacts', 'compte', id]`. Sans cette ligne, un contact créé
       depuis une fiche n'y apparaissait qu'après un rechargement complet — le staleTime est à cinq
       minutes et le rafraîchissement au focus est coupé. Invalider le PRÉFIXE atteint la liste et
       toutes ses dérivées d'un coup. */
